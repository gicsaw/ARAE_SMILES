...
#                Y = tf.layers.batch_normalization(Y, name = 'dis_bn_'+str(i), reuse=reuse)
    ...
    def train(self, x, y, l, n_iter_gan,dev):
        n = np.random.normal(0.0, dev, [self.batch_size, self.latent_size])
        opt1_ = self.sess.run(self.opt1, feed_dict = {self.X : x, self.Y : y, self.L : l, self.N : n})

        for j in range(n_iter_gan):
            s = np.random.normal(0.0, 0.25, [self.batch_size, self.sample_size]).clip(-1.0,1.0)
            for i in range(5):
                opt4_ = self.sess.run(self.opt4, feed_dict = {self.X : x, self.Y : y, self.L : l, self.S : s})
...
#            opt2_ = self.sess.run(self.opt2, feed_dict = {self.X : x, self.Y : y, self.L : l})
    ...
    def test(self, x, y, l, s, n):

        Y, cost1, mol_encoded = self.sess.run(
        [ self.mol_pred, self.reconstr_loss, self.mol_encoded],
        feed_dict = {self.X:x, self.Y : y, self.L :l, self.N: n})

        cost2, cost3, cost4, latent_vector = self.sess.run(
        [ self.g_loss, self.d_loss, self.en_loss, self.latent_vector],
        feed_dict = {self.X:x, self.Y : y, self.L :l, self.N: n, self.S : s}) 

        return Y, cost1, cost2, cost3, cost4, latent_vector,mol_encoded


    def test2(self, x, l, n):
        Y,mol_encoded = self.sess.run([self.mol_pred, self.mol_encoded], 
        feed_dict = {self.X:x, self.L :l, self.N: n})
    ...
    def generate_molecule(self, x, z, l, decoder_state):
        return self.sess.run([self.mol_decoded_softmax, self.decoder_state], feed_dict={ self.X: x, self.mol_encoded : z, self.L : l, self.initial_state : decoder_state}) 

    def generate_latent_vector(self, s):
        return self.sess.run(self.latent_vector, feed_dict={self.S : s}) 

    def get_latent_vector(self, x, n, l):
...
#                print (np.mean(np.array(err_)))
#            if err_<1e-5:
#                break

        retval = self.sess.run(self.S_trials)
#        Y_ = self.sess.run(self.recover_latent_vector)
        print ('check')
#        print (Y_[0][:5])
#        print (latent_vector_real[0][:5])
