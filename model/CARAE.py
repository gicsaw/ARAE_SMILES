...
class ARAE():
    ...
    def _create_network(self):
        ...
        self.en_classified_loss = -tf.reduce_mean(tf.square(predicted_property-self.P)) # need to be modified
        self.classified_loss = tf.reduce_mean(tf.square(predicted_property-self.P))     # need to be modified
...
#        update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
        ...
        self.opt5 = optimizer1.minimize(self.en_classified_loss, var_list = en_list)
        self.opt6 = optimizer1.minimize(self.classified_loss, var_list = pre_list)
...
#            opt2_ = self.sess.run(self.opt2, feed_dict = {self.X : x, self.Y : y, self.L : l})
    ...
    def test(self, x, y, l, s, n, p):
        ...
        [ self.mol_pred, self.reconstr_loss, self.classified_loss, self.en_classified_loss,
        self.mol_encoded],
